...
void TLD::readPathmodel(string& path){
  ifstream fopt(path.c_str(),ios::in);
  if(!fopt){
...
void TLD::read(const FileNode& file){
  ...
  /*
   * I add this to constrain detection forbidding the box move too fast
   */
  speedconstrain = (double)file["speedconstrain"];
...
// add speed constrain
double TLD::getDest(const BoundingBox& box){
  double dst=10000;
  double xc=(box.x+box.width)/2;
  double yc=(box.y+box.height)/2;
  double xc_t=(tbb.x+tbb.width)/2;
  double yc_t=(tbb.x+tbb.height)/2;
  dst=sqrt((xc-xc_t)*(xc-xc_t)+(yc-yc_t)*(yc-yc_t));
  return dst;
}
void TLD::detect(const cv::Mat& frame){
  ...
  int a=0;
  Mat patch;
  for (int i=0;i<grid.size();i++){//FIXME:BootleNeck 
    if(getDest(grid[i])<=speedconstrain){
