...
private:
  ...
  //Parameters
  /* 
   *I add this into tld as a parameter to detection
   */
  double speedconstrain;
  int bbox_step;
  int min_win;
  int patch_size;
...
//Current frame data
  ...
  bool detected;
  //pathmodel
  map<int,vector<double> > pathmodel;
...
public:
  ...
  //Methods
  void readPathmodel(string& path);
  double getDest(const BoundingBox& box);
